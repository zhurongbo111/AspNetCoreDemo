...
namespace Microsoft.AspNetCore.Authentication.OAuth
{
    ...
    public class OAuthOptions : RemoteAuthenticationOptions
    {
        ...
        public override void Validate()
        {
            ...
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "", nameof(ClientId)), nameof(ClientId));
            }

            if (string.IsNullOrEmpty(ClientSecret))
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "", nameof(ClientSecret)), nameof(ClientSecret));
            }

            if (string.IsNullOrEmpty(AuthorizationEndpoint))
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "", nameof(AuthorizationEndpoint)), nameof(AuthorizationEndpoint));
            }

            if (string.IsNullOrEmpty(TokenEndpoint))
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,"", nameof(TokenEndpoint)), nameof(TokenEndpoint));
            }

            if (!CallbackPath.HasValue)
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "", nameof(CallbackPath)), nameof(CallbackPath));
            }
        }
